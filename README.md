...
The goal of the game is to populate the board with a grid of tiles, similar to a crossword, so that all words on the board are from a pre-defined word list. Both players have a rack of up to 7 tiles which is refilled from a scrabbkleTile bag after every move. Each scrabbkleTile has a letter and a numerical value.
...
After that, both players get 7 randomly chosen tiles from the scrabbkleTile bag. The human player is then shown the (currently empty) board and their own tiles. The human player plays the first move.
...
1.
The first move is done by the human player, by combining two or more of their letters to a word. The word is placed on the board to read either to the right or downwards, with one letter on the centre square. Like in Scrabble, diagonal words are not possible in ScraBBKle. Whenever a scrabbkleTile is placed on a square, the letter and the value of the scrabbkleTile replace the square on the board. If the value `S` from the intro is an even number, there are four candidates for the role of "centre square". In ScraBBKle, the top left of these four squares (i.e., the one with minimal column and row) is the centre square for the purposes of the first move.

2.
The game computes the score resulting from the move. As long as the scrabbkleTile bag is not empty, the player who just made a move will have their scrabbkleTile rack topped up with tiles taken from the scrabbkleTile bag so that it has 7 tiles again.
...
5.
Once a scrabbkleTile has been used in a move, its position on the board stays unchanged.
...
8.
In ScraBBKle, it is not allowed to exchange tiles from the scrabbkleTile rack with tiles from the scrabbkleTile bag.
...
10.
The game ends when the scrabbkleTile bag is empty and one of the player has an empty scrabbkleTile rack. The game also ends if both players pass twice in a row.
...
2.
**Premium letter squares** have an integer number of at least `-9` and at most `99` as a factor (specifically, 0 or negative values are possible). When a scrabbkleTile is placed on a premium letter square, the score for the scrabbkleTile is its value multiplied by the factor of the premium letter square. A premium letter square has the shape `(x)` if the factor `x` is a single character and the shape `(xy` if the factor `xy` has two characters.

3.
**Premium word squares** also have an integer number of at least `-9` and at most `99` as a factor (specifically, 0 or negative values are possible). When a move places a scrabbkleTile on a premium word square, the factor of the premium word square will be multiplied with the score obtained for the word otherwise. If a move uses several premium letter squares, the effect is cumulative (for example, when we use a premium word square with factor `4` and a second premium word square with factor `5` in the same move, the resulting factor for the word score would be `4*5 = 20`). Premium word squares are applied only *after* premium letter squares. A premium word square has the shape `{x}` if the factor `x` is a single character and the shape `{xy` if the factor `xy` has two characters.
...
5.
Letter and word premium squares are applied only in a single move. As soon as they have been covered by a scrabbkleTile, in later moves this scrabbkleTile will count at its face value (i.e., the score will not be affected by the premium formerly obtained from covering the scrabbkleTile).
...
The human player also receives new tiles from the scrabbkleTile bag.
...
Here `cr` indicates the column and row of the "origin" of the move. For example, `OND,f8,d` says that from the position in column `f` and row `8` on the board, going `d`own, a scrabbkleTile sequence corresponding to the word `OND` should be played, where occupied squares are skipped. Another example is `VLuE,b12,r`, which says that from position `b12`, going `r`ight, a scrabbkleTile sequence corresponding to the word `VLUE` should be played, where a wildcard should be used for the `U` on the board (indicated by the use of the lower-case letter `u`).
...
When it is the computer player's turn, the move is computed by the program based on the tiles in the computer player's scrabbkleTile rack and the current state of the board. The move is then printed, and the updated board and score are displayed.
...
Information about the release date and the submission deadline for this coursework assignment is available behind the *Assessment* scrabbkleTile of the Moodle website for the *Programming in Java* module.
